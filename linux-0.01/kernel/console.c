...
volatile unsigned int f1_pressed = 0;
...
void clipboard_selected_item_down(){
	if((clipboard_selected_item + 1) > 9){
...
void con_write(struct tty_struct * tty)
{
	...
	set_cursor();
	if(f2_pressed){
		tool_start(); // re-draw tool after each input
	}
}
